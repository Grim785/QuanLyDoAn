// Hàm xử lý "select all" và chọn từng dòng
  ...
  function initializeSearchHandler() {
    document.getElementById('search-bar').addEventListener('input', (event) => {
      ...
      fetch(`/admin/advisor/search?query=${query}`)
          ...
          data.forEach((student) => {
            const row = `
                    <tr>
                      ...
                      <td>${student.id}</td>
                      <td><input type="text" class="form-control form-control-xs cell-input" value="${student.advisorID}" /></td>
  ...
  function initializeAddStudentHandler() {
    ...
    document.getElementById('save-button').addEventListener('click', () => {
      ...
      const advisorID = document.getElementById('new-studentID').value.trim();
  ...
  function initializeDeleteSelectedHandler() {
    document.getElementById('delete-button').addEventListener('click', () => {
      ...
      if (confirm(`Bạn có chắc chắn muốn xóa ${selectedIds.length} giảng viên này không?`)) {
        // Gửi yêu cầu xóa đến backend
        fetch('/admin/adviors/delete', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
          ...
          .then((data) => {
            if (data.error) {
              ...
              alert(data.message || 'Xóa giảng viên viên thành công!');
  ...
  document.getElementById('edit-button').addEventListener('click', async () => {
    ...
    const updatedStudents = selectedCheckboxes.map((checkbox) => {
      ...
      const advisorID = row.querySelector('[name="studentID"]').value.trim();
      ...
      return { id, advisorID, lastname, firstname, date_of_birth, gender, address, username };
    });
    ...
    // Gửi yêu cầu AJAX
    try {
      const response = await fetch('/admin/adviors/update', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (response.ok) {
        alert(result.message || 'Cập nhật giảng viên thành công!');
